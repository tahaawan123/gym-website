...
const Header = () => {
  ...
  return (
    <header className="bg-black bg-opacity-80 text-white h-16 flex items-center justify-between px-5 md:px-10 z-50 shadow-lg w-full fixed top-0">
      ...
      {/* Desktop Menu */}
      <nav className="hidden md:flex space-x-6 lg:space-x-10 text-sm lg:text-lg">
        {['Home', 'MemberShip', 'Our Branches', 'About Us', 'Contact'].map((name, index) => (
          <Link key={index} href="#" className="hover:text-yellow-400 transition duration-200">
            {name}
          </Link>
        ))}
      </nav>
      ...
      <div className="hidden md:block">
        <button className="px-4 py-2 bg-yellow-300 text-black rounded-lg hover:border-l-4 border-yellow-500 transition duration-300">
          Join Now
        </button>
      </div>
